...
Live analysis uses Canny contours and a four-corner approximation, with a convex-hull/relaxed approximation fallback for modest gutter or finger irregularity. Wide candidates and equally plausible distinct pages prompt centering one page. Normalized corners map directly into an SVG sharing the video's contain fit. The fixed guide becomes quiet once a strong page outline appears. A 480px perspective-normalized grayscale preview uses adaptive thresholding and horizontal morphology to identify line-like components; `textBody.ts` merges nearby lines into one main region. Its corners map back through the inverse perspective transform. This is a visual estimate, not live OCR or semantic layout understanding. A missing body never disqualifies a detected page. When a usable page border is absent, the same worker inspects the guide crop for text instead. At least three distinct line-like rows, a body at least 25% wide and 12% tall, and clear guide margins permit automatic capture with `source: text`. Clipped lines, blank scenes, or ambiguous spreads do not qualify. This mode uses the full guide crop and does not invent a perspective transform from a text rectangle. The text outline remains visible even without page corners.

`AutoScanMachine` requires 750 ms and at least three samples of stable corners, area, normalized page content (guide content when unavailable), and text-body position when available, plus alignment/sharpness/brightness. The detected region shows an animated scanning sweep plus a progress outline and accessible progress bar; reduced-motion settings disable the sweep. Accepted geometry is frozen for a 500 ms regional green flash with a checkmark and status text. A separate timer expires feedback even when preview sampling is paused. OCR starts in the background immediately after reservation; its completion does not control the flash or page numbering.
