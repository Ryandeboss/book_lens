...
The worker uses a detected page or central preview region to measure brightness and Laplacian variance. Dark or blurry frames skip text contours. A roughly page-shaped outline or printed line components establish page presence; neither blank margins, guide alignment nor text touching camera edges block capture. Text bounds remain diagnostic data and do not size the animation or crop the shot. This is local visual evidence with no preview OCR or network request.
