...
CameraPreview video (useCamera owns tracks)
   ...
   |       | motion -> page ROI -> lighting/focus -> optional text overlay
   |       v
   |   AutoScanMachine: alignment + stability + two-second capture pause
   |       |
   +-------+-> ImageCapture photo or video Blob, capped at 8 MP
        ...
        high-quality JPEG (0.94) or PNG when smaller
               |
        compact fingerprints retained; no visual duplicate rejection
               |
        reserve shot position; green confirmation; pause capture for 2 seconds
               |
        App-owned OCR queue (max 30 images / 48 MiB, active job included)
               |
        POST /api/ocr -> Render/Express -> Google Document AI v1
               ...
               +----------------------------+
               |
        optional /api/proofread -> OpenAI text cleanup
               |
        update original page ID; detect duplicate text; release image
               |
...
`textDuplicates.ts` compares raw OCR, never AI output. It requires at least 50 words and a 95% length ratio; exact normalized text or a multiset five-word-shingle Dice score of at least 96% qualifies. Different numeric sequences veto fuzzy matches. This deliberately misses short/uncertain duplicates rather than discarding distinct pages. The scan store recomputes matches in capture order when OCR completes, including out-of-order completions. The first matching shot remains included; later matches get a `duplicateOf` ID and score. Review can restore each match at its original position. Deleting an original promotes the next matching shot. `capturePosition` never changes after deletion, while display page numbers retain their existing renumbering behavior.
