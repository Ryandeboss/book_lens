...
CameraPreview video (useCamera owns tracks)
   |
   +-> 640px analysis frame, at most ~6/sec; video-frame callback or timer
   |       |
   |       v
   |   usePageDetection -> OpenCV Web Worker
   |       | motion -> page ROI -> lighting/focus -> text -> visual signatures
   |       v
   |   AutoScanMachine: alignment + stability + page-change lock
   |       |
   +-------+-> ImageCapture photo or video Blob, capped at 8 MP
               |
               v
...
Live analysis fails early in a fixed order. A 64x64 grayscale sample compares consecutive views with exposure-centered mean absolute difference. It uses the last reliable page region, or the frame when no page is known; changing the ROI/resolution resets its baseline. Motion skips page contours, Laplacian, and text analysis. Canny contours, quadrilateral approximation and a relaxed convex hull pass then find visible page corners. Area, aspect, guide alignment, ambiguity and opposite-edge ratios reject clipped/tiny/extremely skewed or two-page candidates while tolerating modest curl/gutter irregularities. A central guide ROI supplies fallback analysis when the paper boundary is weak.

The worker perspective-normalizes the page ROI to at most 480px and measures brightness and Laplacian variance in its inset interior. Dark or blurry frames skip adaptive thresholding and text contours. Gaussian adaptive thresholding plus horizontal morphology forms line components. `textBody.ts` groups nearby aligned rows and estimates one padded body rectangle. Three rows, minimum body area/size and whitespace margin evidence qualify fallback text. Strong page geometry and focus can qualify sparse title/illustration/footnote pages even when dense text evidence is uncertain. This is local visual evidence, with no preview OCR, ML runtime, TextDetector, or network request.

The existing state machine requires both at least three consecutive acceptable samples and 450ms of stable corners, scale, body position and content. Any failed gate resets readiness. A geometry change or sample gap restarts the window. Ordinary guidance is debounced 220ms; shutter/success/error/pause messages remain immediate. Capture takes a lock before asynchronous work, performs the existing recent-page duplicate check, then reserves the next page number. The frozen accepted region flashes green for 500ms; OCR is not awaited.
...
After green feedback, the machine stays locked until consecutive visual changes indicate a turn. Page-normalized grayscale, hash, edges and text-density signals prevent minor brightness changes from immediately rearming it. Turns are observed even during green feedback and queue backpressure. Rearming still requires fresh stability, and recent duplicate matching independently rejects known pages. The queue keeps the existing one-to-two OCR concurrency and bounded images; available capacity automatically resumes capture.
