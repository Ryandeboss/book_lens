...
Pinia stays authoritative while the app is open. `scanDraft.ts` subscribes to document changes and the OCR queue image lifecycle, with a 400ms debounce and a two-second maximum wait. Visibility/pagehide events flush pending changes. `scanDraftDb.ts` stores metadata and a separate bounded set of image Blobs atomically in IndexedDB; unchanged photos are not copied again on text edits. Raw OCR completion removes the recovery image, even while text-only cleanup is pending.

Startup loads metadata and removes expired or incompatible records. A recovery prompt gates the scanner until the user resumes or discards. Only Resume loads retained Blobs and restores Pinia; completed pages keep their text/edits, pending images rejoin the existing queue under their original IDs, missing images become rescan errors, and interrupted cleanup stays available for an explicit retry without rerunning OCR. Capture sequence survives deleted pages. Camera access remains user-triggered.

A draft expires 24 hours after its last save; cleanup runs at startup and on an open-tab timer. Explicit Start New/Finish/Discard removes metadata content and all Blobs. Scanner Done retains the draft for Review; downloading alone does not clear it. A small revision tombstone prevents stale tabs or queued writes from reviving a cleared draft. Transactions use optimistic revisions to stop another tab overwriting newer work. Quota/unavailable-storage errors leave Pinia usable with a visible warning. This is local recovery, not a durable backup, cloud sync, background OCR or an offline app. See [recovery behavior](draft-recovery.md).
