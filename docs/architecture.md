...
Live analysis is now text-first across the entire low-resolution preview. Adaptive thresholding and horizontal morphology form line-like components. Contour retrieval includes nested text inside paper borders. Enclosing paper/background contours are excluded from the margin mask, while text and local illustrations remain, so a tilted paper edge cannot masquerade as printed ink. `textBody.ts` groups nearby lines into one rectangle and measures actual ink in four surrounding whitespace bands. At least three rows and a region at least 25% wide / 12% tall are required; each margin band must contain at most 8% ink. Paper contours, page aspect, and the old fixed portrait guide no longer gate automatic capture.

Focus (Laplacian variance) and brightness are measured inside the text region. The state machine reports missing light, blur, or insufficient margin and automatically captures after 450 ms / three acceptable samples with modest geometry stability. Text-mode readiness does not reset on noisy whole-frame intensity differences. A scanning sweep/progress outline shows readiness and the accepted text region flashes green for 500 ms. The fixed guide is removed. Reduced-motion preferences disable the sweep.

Automatic capture passes full-frame corners to the existing image-processing worker, preserving visible headings/footnotes instead of cropping OCR to the detected block. The optional text region is transformed into the corrected image only for compact signatures and ORB comparison. The full image is still compressed and queued for Google, with Tesseract fallback. The text rectangle describes a visual cluster, not an OCR interpretation; aim at one page and keep unrelated text out of the photo. Sparse single headings or image-heavy pages without a clear multi-line block can use Manual Capture.

Two protections remain separate. The held-page lock compares normalized page content using mean-centered grayscale, edge difference, a 64-bit difference hash, and a 4x6 ink-density grid. Two changed signals in two consecutive samples rearm scanning; normalized content avoids mistaking phone translation for a new page. If the page is absent, the existing guide signature observes a physical turn. Turns are observed even during the flash or queue backpressure. Switching between paper-edge and text-only detection uses the common guide signature for the held-page lock, preventing that geometry change alone from being interpreted as a turn. Rearming still requires a new stability window.
