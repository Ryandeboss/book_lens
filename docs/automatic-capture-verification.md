...
3. The worker samples 64x64 grayscale pixels in the last reliable page region, or the frame before a page is known. It subtracts the mean exposure difference, then computes average absolute residual / 255. Above 0.022 blocks the frame before contour, sharpness or text work. New dimensions/ROI require a new baseline. Corner translation, area and perspective changes remain secondary stability signals.
...
| 8. Slight hand shake                 | Wait until steady; `motionDifference` (0.022), `cornerMovement` (0.025), `pageAreaMovement` (0.1) control tolerance.                              |
...
| 10. Mostly text                      | Body overlay and progress appear; fallback uses `textCaptureMinLines` (3), minimum body dimensions and margin ink (0.08).                         |
