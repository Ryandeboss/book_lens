...
Pinia remains the live document state. The active scan is autosaved to IndexedDB on this browser/device and offered through **Resume previous scan?** after refresh or restart. Drafts expire after roughly 24 hours without changes. Text, edits, OCR structure/provider, duplicates and capture order are retained; only photos awaiting OCR or within the bounded retry cache are stored. OpenCV runs in a browser worker. Corrected page images are temporarily uploaded to Render, then Google Document AI. The backend does not persist them; the local recovery draft removes a photo once its OCR text is saved. Tesseract is the fallback, not the preferred primary engine. Finishing the background job releases the queued image. Pending and retry images have separate count and byte limits. **Done** opens Review and keeps the draft. **Finish scan**, **Start New Scan**, and **Discard draft** clear recovery data. Download TXT for a permanent copy; browser storage can be unavailable or cleared. See [draft recovery](docs/draft-recovery.md).
