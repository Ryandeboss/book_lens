...
const schema = z.object({
  ...
  TTS_LANGUAGE_CODE: z.string().trim().min(1).default('en-US'),
  TTS_VOICE_NAME: z.string().trim().min(1).default('en-US-Standard-C'),
  TTS_TIMEOUT_MS: z.coerce.number().int().min(1000).max(60000).default(45000),
  TTS_REQUESTS_PER_MINUTE: z.coerce.number().int().min(1).max(1000).default(60),
